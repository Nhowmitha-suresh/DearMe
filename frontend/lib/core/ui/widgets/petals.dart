...
class FloatingPetals extends StatefulWidget {
  final int count;
  final double width;
  const FloatingPetals({Key? key, this.count = 8, this.width = 300})
      : super(key: key);
...
class _PetalPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < count; i++) {
      final angle =
          (i / count) * 2 * pi + progress * 2 * pi * (0.5 + rand.nextDouble());
      final radius = size.width * (0.18 + 0.22 * sin(progress * 2 * pi + i));
      final petalCenter =
          center + Offset(cos(angle) * radius, sin(angle) * radius);
      final paint = Paint()
        ..color = Color.lerp(
                const Color(0xFFFFCFE6), const Color(0xFFFFE6EE), i / count)!
            .withOpacity(0.9 - 0.3 * (i / count));
      canvas.drawOval(
          Rect.fromCenter(
              center: petalCenter,
