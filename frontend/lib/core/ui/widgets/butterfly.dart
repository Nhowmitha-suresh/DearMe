...
class _ButterflyPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // left wing
    paint.color =
        Color.lerp(const Color(0xFFFFA3C4), const Color(0xFFFFD7EB), t)!;
    final lPath = Path();
    lPath.moveTo(center.dx, center.dy);
    lPath.quadraticBezierTo(
        center.dx - size.width * 0.7,
        center.dy - size.height * 0.9 * (0.6 + 0.4 * sin(t * pi)),
        center.dx - size.width * 0.1,
        center.dy - size.height * 0.05);
    lPath.quadraticBezierTo(center.dx - size.width * 0.4,
        center.dy + size.height * 0.7, center.dx, center.dy);
    canvas.drawPath(lPath, paint);

    // right wing
    paint.color =
        Color.lerp(const Color(0xFFFFE6EE), const Color(0xFFFFCFE6), 1 - t)!;
    final rPath = Path();
    rPath.moveTo(center.dx, center.dy);
    rPath.quadraticBezierTo(
        center.dx + size.width * 0.7,
        center.dy - size.height * 0.9 * (0.6 + 0.4 * cos(t * pi)),
        center.dx + size.width * 0.1,
        center.dy - size.height * 0.05);
    rPath.quadraticBezierTo(center.dx + size.width * 0.4,
        center.dy + size.height * 0.7, center.dx, center.dy);
